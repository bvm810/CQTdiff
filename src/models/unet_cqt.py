...
class Upsample(nn.Module):
    ...
    def __init__(self, S):
        ...
        self.resample = torchaudio.transforms.Resample(
            N, N * S
        )  # I use 3**12 as an arbitrary number, as we don't care about the sampling frequency of the latents
...
class Downsample(nn.Module):
    ...
    def __init__(self, S):
        ...
        self.resample = torchaudio.transforms.Resample(
            N, N / S
        )  # I use 2**12 as an arbitrary number, as we don't care about the sampling frequency of the latents
...
class AddFreqEncoding(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        fembeddings_2 = torch.broadcast_to(
            self.fembeddings, [batch_size_tensor, time_dim, 2, self.f_dim, 10]
        )
...
class CpxFilm(nn.Module):
    ...
    def forward(self, sigma_encoding):
        ...
        sigma_encoding = sigma_encoding.unsqueeze(
            -1
        )  # we need a secnond unsqueeze because our data is 2d [B,C,1,1]
...
class ResnetBlock(nn.Module):
    def __init__(
        ...
        self.res_conv = (
            nn.Conv2d(dim, dim_out, 1, padding_mode="reflect", bias=bias)
            if dim != dim_out
            else nn.Identity()
        )
        ...
        self.first_conv = nn.Sequential(
            nn.GELU(), nn.Conv2d(dim, dim_out, 1, bias=bias)
        )

        for i in range(self.num_layers):
            self.H.append(
                Gated_residual_layer(dim_out, (5, 3), (2**i, 1), bias=bias)
...
class Unet_CQT(nn.Module):
    ...
    def __init__(self, args, device):
        ...
        self.init_conv = nn.Conv2d(
            Nin, self.Ns[0], (5, 3), padding="same", padding_mode="reflect", bias=False
        )
            ...
            if i > 0:
                self.ups.append(
                    nn.ModuleList(
                        [
                            ResnetBlock(
                                dim_in, dim_out, use_norm=self.use_norm, bias=False
                            ),
                            Upsample(self.Ss[i]),
