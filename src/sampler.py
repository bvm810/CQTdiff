...
class Sampler:
    def __init__(
        self, model, diff_params, args, xi=0, order=2, data_consistency=False, rid=False
    ):
        self.model = model
        ...
        self.data_consistency = (
            data_consistency  # use reconstruction gudance without replacement
        )
        self.args = args
    ...
    def data_consistency_step_phase_retrieval(self, x_hat, y):
        ...
        # print(x.shape)
        x2 = torch.cat(
            (x_hat, torch.zeros(x_hat.shape[0], win_size).to(x_hat.device)), -1
        )
        X = torch.stft(
            x2,
            win_size,
    ...
    def get_score_rec_guidance(self, x, y, t_i, degradation):
        ...
        if self.treshold_on_grads > 0:
            ...
            rec_grads = torch.clip(
                rec_grads, min=-self.treshold_on_grads, max=self.treshold_on_grads
            )
    ...
    def predict(
        ...
        for i in tqdm(range(0, self.nb_steps, 1)):
            ...
            if t[i + 1] != 0 and self.order == 2:  # always except last step
                ...
                t[i + 1] == 0 or self.order == 1
            ):  # first condition  is to avoid dividing by 0
...
class SamplerCompSens(Sampler):
    ...
    def predict_compsens(self, y_masked, mask):
...
class SamplerDeclipping(Sampler):
    ...
    def predict_declipping(self, y_clipped, clip_value):
...
class Sampler78rpm(Sampler):
    def __init__(
        self, model, diff_params, args, xi=0, order=2, data_consistency=False, rid=False
    ):
        super().__init__(model, diff_params, args, xi, order, data_consistency, rid)
        self.device = args.device
        self.get_noise_samples()

    def get_noise_samples(self):
        # samples need to be in the same sampling rate as original audio !!!
        noise_folder = self.args.inference.denoising.noise_files_path
        noise_paths = [
            os.path.join(noise_folder, file) for file in os.listdir(noise_folder)
        ]
        self.noises = [
            utils_denoising.get_noise(
                n_samples=self.args.audio_len,
                crossfade_time=self.args.inference.denoising.crossfade_duration,
                noise_path=path,
            )
            for path in noise_paths
        ]

    def add_noise(self, signal, gain):
        noise = random.choice(self.noises).to(self.device)
        noise_power = torch.sum(torch.pow(noise, 2)) / noise.shape[-1]
        noise = noise / torch.sqrt(noise_power)
        noisy_signal = utils_denoising.add_noise_with_gain(signal, noise, gain)
        return noisy_signal

    def add_noise_with_snr(self, signal, range):
        noise = random.choice(self.noises).to(self.device)
        snr = random.uniform(range[0], range[1])
        noisy_signal = utils_denoising.add_noise_with_snr(signal, noise, snr)
        return noisy_signal

    def add_noise_with_variable_xi(self, signal, range):
        noise = random.choice(self.noises).to(self.device)
        snr = random.uniform(range[0], range[1])
        self.xi = max(0.35, 0.105 * range[0] - 0.7)
        noisy_signal = utils_denoising.add_noise_with_snr(signal, noise, snr)
        return noisy_signal

    def predict_gramophone_denoising(self, y_noisy, gain):
        degradation = lambda x: self.add_noise(x, gain)
        if self.rid:
            res, denoised, t = self.predict_conditional(y_noisy, degradation)
            return res, denoised, t
        else:
            res = self.predict_conditional(y_noisy, degradation)
            return res

    def predict_gramophone_denoising_with_snr(self, y_noisy, range):
        degradation = lambda x: self.add_noise_with_snr(x, range)
        if self.rid:
            res, denoised, t = self.predict_conditional(y_noisy, degradation)
            return res, denoised, t
        else:
    ...
    def predict_gramophone_denoising_with_variable_xi(self, y_noisy, range):
        degradation = lambda x: self.add_noise_with_variable_xi(x, range)
        if self.rid:
            res, denoised, t = self.predict_conditional(y_noisy, degradation)
            return res, denoised, t
        else:
...
class SamplerInpainting(Sampler):
    ...
    def predict_inpainting(self, y_masked, mask):
        self.mask = mask.to(y_masked.device)
...
class SamplerBWE(Sampler):
    ...
    def apply_biquad(self, y):
        y_lpf = torchaudio.functional.biquad(
            y, self.b0, self.b1, self.b2, self.a0, self.a1, self.a2
        )
        return y_lpf
    ...
    def resample(self, x):
        ...
        return torchaudio.functional.resample(
            x, orig_freq=int(N * self.factor), new_freq=N
        )
    ...
    def predict_bwe(
        ...
        if filt_type == "firwin":
            ...
            return self.predict_resample(
                ylpf, (ylpf.shape[0], self.args.audio_len), degradation
            )
