...
class Learner:
    def __init__(self, model_dir, model, train_set, args, log=True):
        """
        Args:
        ...
        if args.sde_type == "VE_elucidating":
            self.diff_parameters = VE_Sde_Elucidating(
                args.diffusion_parameters, args.diffusion_parameters.sigma_data
            )
        else:
        ...
        if self.log:
            ...
            wandb.init(
                project=args.wandb.project, entity=args.wandb.entity, config=config_dict
            )
        ...
        if S > 2.1 and S < 2.2:
            ...
            self.resample = torchaudio.transforms.Resample(N, self.args.audio_len).to(
                self.device
            )
    ...
    def get_data_batch(self):
        ...
        # making sure it does not resample to 22050
        ...
        wandb.log(
            {
                ...
                + str(string): wandb.Audio(
                    audio_path, sample_rate=self.args.sample_rate
                )
            },
            step=self.step,
        )
    ...
    def _write_summary(self):
        ...
        quantized_sigma = (
            (sigma ** (1 / ro) - sigma_max ** (1 / ro))
            * (self.n_bins - 1)
            / (sigma_min ** (1 / ro) - sigma_max ** (1 / ro))
        )
        ...
        mean_grad_norms = (
            self.cum_grad_norms / num_elems_in_bins.sum() * self.args.batch_size
        )
    ...
    def update_accumulated_loss(self, vectorial_loss, sigma_array, isTrain):
        if (self.accumulated_losses is None) and (
            self.accumulated_losses_sigma is None
        ):
            self.accumulated_losses = vectorial_loss
            self.accumulated_losses_sigma = sigma_array
        else:
            assert not (
                (self.accumulated_losses is None)
                or (self.accumulated_losses_sigma is None)
            ), "either both or none of them should be None"
            self.accumulated_losses = np.concatenate(
                (self.accumulated_losses, vectorial_loss), axis=0
            )
            self.accumulated_losses_sigma = np.concatenate(
                (self.accumulated_losses_sigma, sigma_array), axis=0
            )
