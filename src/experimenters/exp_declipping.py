...
class Exp_Declipping(Exp_Base):
    def __init__(self, args, plot_animation):
        ...
        self.sampler = SamplerDeclipping(
            self.model,
        ...
        self.__filter_final = utils_bwe.get_FIR_lowpass(
            100, 10000, 1, self.args.sample_rate
        )
        self.__filter_final = self.__filter_final.to(self.device)

        # clipping specific
        # n = "clipped" + "_" + str(self.args.inference.declipping.SDR) # original
        n = "clipped" + "_" + str(self.args.inference.declipping.clip_value)  # modified
        self.path_degraded = os.path.join(
            self.path_sampling, n
        )  # path for the clipped outputs
        ...
        # n = "declipped" + "_" + str(self.args.inference.declipping.SDR) # original
        n = "declipped" + "_" + str(self.args.inference.declipping.clip_value)  # mod
    ...
    def conduct_experiment(self, seg, name):
        ...
        # original code below, changed this for fixed clip value from args
        # clip_value=utils_declipping.get_clip_value_from_SDR(seg,self.args.inference.declipping.SDR)

        clip_value = self.args.inference.declipping.clip_value
