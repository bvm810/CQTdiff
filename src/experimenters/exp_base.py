...
class Exp_Base:
    def __init__(self, args):
        ...
        if self.args.architecture == "unet_CQT":
            ...
            print(f"Loading from {args.inference.checkpoint}")
            self.model = load_ema_weights(
                self.model, os.path.join(args.model_dir, args.inference.checkpoint)
            )
        ...
        self.path_sampling = os.path.join(
            args.model_dir, self.args.inference.mode + today.strftime("%d_%m_%Y")
        )
