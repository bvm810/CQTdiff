...
class TrainDataset(torch.utils.data.IterableDataset):
    def __init__(self, dset_args, fs=44100, seg_len=131072, seed=42):
        ...
        # original moliner filter for years in maestro dataset
        ...
        self.train_samples = [os.path.join(path, file) for file in os.listdir(path)]
    ...
    def __iter__(self):
        while True:
            ...
            assert samplerate == 44100  # making sure its not 22050
            data_clean = data
            # Stereo to mono
            if len(data.shape) > 1:
                data_clean = np.mean(data_clean, axis=1)
